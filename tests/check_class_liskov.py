...
def check(by_name):
    ...
    for name, cls in sorted(by_name.items()):
        ...
        # context/spec_examples/spec_constraints (#323 + Job ruling; CLS-003): the served tiers —
        # every type, and every base a producer may bind at (CLS-002 (c)). Never a provider class,
        # and never a folder (`instantiable: false`), which has no served surface of its own.
        for key in ("context", "spec_examples", "spec_constraints"):
            if not cls.get(key):
                continue
            tier = cls.get("class")
            if tier == "provider" or (tier == "base" and cls.get("instantiable") is False):
                why = "provider tier carries no served prose" if tier == "provider" else                       f"a folder ({name} is `instantiable: false`) has no served surface — its members own it"
