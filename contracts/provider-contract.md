...
3. **Config-projection detail** — enough config schema/detail for the **DCM interface to project a configuration interface to the user across the config lifecycle**, at the provider's supported scale (basic text passthrough → typed; DCM ADR-023 §6). No detail → DCM still projects a basic text passthrough; more detail → a typed interface. **If the provider exposes its OWN editor** (DCM delegates editing to it rather than projecting one), the contract binds the provider to keep the audit loop closed — a provider editor is **not** an audit bypass: it MUST **(a)** report the resulting **realized-state updates** back to DCM (denaturalized, per-resource, §1a.5 read-back) so DCM records the new config **state** (UDLM is the state system-of-record — ADR-016 §3), **(b)** support DCM's **before-and-after actor validation** — DCM **authorizes the actor before** the edit (the PDP resolution, item 8) and **attributes the applied change to that same validated actor after**; and **(c)** honor **sovereignty and tenancy** — the config tooling operates **inside** DCM's governance, never around it: the Governance Matrix fires on the edit (data-residency / sovereignty hard-gate, §4), config data carries its `data_classification` and `tenant_uuid`, an actor edits **only within their tenant**, and a change that would cross a jurisdiction is gated exactly as any other boundary crossing. **Audit, observability, and provenance hold at all times**: no config change reaches Realized without an authorized in-tenant actor, a governance-cleared edit, a read-back, and an audit leaf.
