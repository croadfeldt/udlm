...
| `PRV-010` | **Resource-type extension (ADR-PROV-004, closes #198).** A provider MAY extend a resource type it realizes by ADDING data elements, but MUST NOT override, shadow, or redefine any base element — the base type-spec is closed (`additionalProperties: false`). **Vendor-neutral-first (the extension gate).** An extension is admissible ONLY for genuinely non-portable data that has **no existing vendor-neutral definition** — not in the base spec, not in an adopted standard (T5), and not expressible as a portable form. If a vendor-neutral definition already covers the datum, the provider MUST use it, never an extension; DCM **rejects at registration** an extension element that duplicates an existing vendor-neutral definition, directing it to the neutral form (or, if recurrent across ≥2 providers, to a base MINOR). This upholds the naturalization boundary (`foundations/context-and-purpose.md` §7.1): the substrate never translates or stores a native spec; a vendor extension is only the *store-and-forward* of genuinely-non-portable data whose data definition the vendor owns — the last resort, not the default. It **declares** its extensions at registration (which base type + the added elements and their schema), and at realization writes them ONLY into the realized entity's provider-namespaced `provider_extensions` surface; the validator rejects any extension path colliding with a base spec field. Any extension **degrades portability**: DCM computes `portability_breaking: true`, narrows the classification, records the extension keys + bound provider, and **notifies the consumer** before/at realization (silent non-portability prohibited). A genuinely new type is a Tier-2 `Vendor.Type` fork, not an extension; a recurrence across ≥2 providers promotes to a base MINOR. |
