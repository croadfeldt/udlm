...
**In one breath.** `Container` and `Compute.VM` are **peer resource types** in one
...
```mermaid
flowchart TD
  INT["Intent — Container OR Compute.VM<br/>same shape: image · resources · networks · storage · placement"] --> GRAPH["Dependency graph<br/>the same typed edges regardless of form factor"]
...
- **Container is a peer type, not a special case.** `Container` sits beside `Compute.VM`
