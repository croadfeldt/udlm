...
**In one breath.** `Container` and `Machine.VM` are **peer resource types** in one
...
```mermaid
flowchart TD
  INT["Intent — Container OR Machine.VM<br/>same shape: image · resources · networks · storage · placement"] --> GRAPH["Dependency graph<br/>the same typed edges regardless of form factor"]
...
- **Container is a peer type, not a special case.** `Container` sits beside `Machine.VM`
...
- A `Container` is declared with the **same intent shape** as a VM and validated against the same
  meta-schema.
