...
```mermaid
flowchart TD
  D["Define type<br/>Machine.VM<br/>spec + lifecycle intent + status"] --> S["Submit a VM intent<br/>(persisted as a typed resource)"]
...
- UDLM defines a `Machine.VM` resource with spec (placement, resources, storage, networks), lifecycle intent, and status.
...
- **Data:** the `Machine.VM` type — portable spec base, lifecycle intent field, and a status subresource for realized facts.
...
- Base flow: [request-realization](request-realization.md). UC source: `compute/vm-resource-representation`.
