...
```mermaid
flowchart TD
  D["Define type<br/>Compute.VM<br/>spec + lifecycle intent + status"] --> S["Submit a VM intent<br/>(persisted as a typed resource)"]
...
- UDLM defines a `Compute.VM` resource with spec (placement, resources, storage, networks), lifecycle intent, and status.
...
- **Data:** the `Compute.VM` type — portable spec base, lifecycle intent field, and a status subresource for realized facts.
