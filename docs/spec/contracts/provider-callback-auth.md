...
This document specifies the **substrate contract** by which Service Providers authenticate inbound calls to a UDLM-conformant implementation — specifically, calls to the provider callback surface:
...
When an implementation receives a callback, it MUST verify five properties:
...
Network-level authentication alone (firewall rules, IP allowlisting) is insufficient under the substrate's zero-trust model — it establishes perimeter trust, not per-call identity. Every provider call to a UDLM implementation MUST carry credentials that answer all five questions independently of network position.

---

## 3. Two-Layer Authentication Contract (Abstract)

The substrate REQUIRES that a UDLM-conformant implementation validate every provider callback via **two independent identity factors**:
...
```
Provider calls peer implementation:
  │
  ▼ Layer 1: Identity Attestation Factor
  │   Provider presents its identity attestation
  │   The implementation validates the attestation against the provider's registration
  ...
  │   The implementation validates: factor is active, scoped to this provider, scoped to this operation type
...
**Mechanism declaration:** Each implementation MUST declare its chosen Layer 1 and Layer 2 mechanisms via the [schema-sharing protocol](schema-sharing.md) so federation peers and providers can interoperate.

---

## 4. Provider Identity Attestation Contract (Layer 1)

The substrate requires that a UDLM implementation attest provider identity at registration via a verifiable mechanism. The implementation's chosen mechanism MUST satisfy the following contract properties:

### 4.1 Registration-Time Identity Binding

At registration, the implementation MUST capture verifiable identity-binding material for the provider. This material MUST be:
...
- Subject to rotation (the implementation MUST support rotation of identity material on a declared interval)
- Subject to revocation (the implementation MUST be able to revoke the identity binding)

The specific format (X.509 certificate, signed JWT key pair, HSM key, ed25519 signing key, etc.) is an implementation choice. The provider's registration record MUST include a mechanism declaration so federation peers can resolve verification.

### 4.2 Inbound Identity Verification

On every subsequent inbound connection, the implementation MUST verify the presented Layer 1 factor:
...
The substrate defines the following properties of the Layer 2 callback credential. The cryptographic format (opaque bearer token, signed JWT, capability macaroon, etc.) is an implementation choice.

```yaml
callback_credential:
  credential_uuid: <uuid>
  credential_type: <substrate-defined: dcm_interaction (or implementation-declared equivalent)>
  issued_to:
...
**Key substrate property:** The credential is scoped to the `provider_uuid` — not to specific entities or operations within that provider. Entity-level scope is enforced separately (Section 6). A provider holding the credential can call any callback endpoint in its scope, but the implementation MUST enforce entity-level ownership checks per call.

### 5.2 Credential Lifecycle (Technology-Neutral)

The substrate requires the following credential lifecycle states. Specific issuance, delivery, and rotation mechanisms are implementation choices.

```
Registration approved (provider status → ACTIVE)
  │
  ▼ Implementation issues Layer 2 callback credential
  ...
  │   Delivery mechanism is implementation-declared
  ...
  │
  ▼ Pre-expiry rotation
  │   Implementation issues a new credential before expiry
...
The substrate requires that lifetime monotonically decrease with stricter profiles; specific values MAY vary per implementation.

---

## 6. Authentication-at-Callback-Time Contract

Every callback MUST present both Layer 1 and Layer 2 factors. The receiving implementation MUST validate both before accepting the call. The substrate-required validation sequence is:
...
For `realized_state_push` and `interim_status` calls, the implementation MUST validate:

1. Look up the Requested State record for the entity
2. Verify the credential's `provider_uuid` matches the `provider_uuid` in the Requested State record (i.e., this was the provider the implementation dispatched to)
...
For `update_notification` calls, the implementation MUST validate:
...
For `lifecycle_event` calls, the implementation MUST validate:
...
Platform admin (or automation) triggers revocation:
  │
  ▼ The implementation revokes the credential immediately
  ...
  │   Active implementations enter PENDING_REVIEW state
  │
...
| Policy | Rule |
|--------|------|
| `PCA-001` | All provider-to-implementation calls MUST present both a valid Layer 1 identity attestation factor and a valid Layer 2 callback credential. Neither layer alone is sufficient. |
...
| `PCA-005` | Callback credentials are issued by a registered Credential Provider, not directly by the implementation's API surface. The Credential Provider is the authoritative source for all credential issuance, rotation, and revocation. |
...
| `PCA-008` | Callback credentials MUST be rotated before expiry. The implementation MUST initiate rotation automatically. If a credential expires without rotation, the provider enters a CREDENTIAL_EXPIRED state and must obtain a new credential via the platform admin. |
...
| `PCA-011` | The specific mechanisms backing Layer 1 and Layer 2 are implementation choices. Each implementation MUST declare its chosen mechanisms via the [schema-sharing protocol](schema-sharing.md) so federation peers can interoperate. |
