...
A resource's relationships arise at two different points in the lifecycle, and the contract must not conflate them. This is the distinction that settles "who defines the relationship — the request or the provider?": **both do, at different times, about different things** (UDLM DCM ADR-009).
...
A type that supports **neither** — where a genuinely-bespoke field is required — is **non-conformant for brokered fulfillment**. *Note:* `Network.IPAddress` is **not** such a case: it needs only the shared segment reference above, so it requires no extension and no derived type. Accommodation applies only where the base type cannot carry provider-specific realize-time state at all. See UDLM DCM ADR-009 for the end-to-end flow.
...
service_provider_capabilities:
  ...
  dependency_introspection:
    ...
    # consulted for orchestration. See docs/spec/foundations/service-dependencies.md §3a
  ...
  # scoring and cost attribution (docs/spec/contracts/cost-metering-linkage.md — "the engine computes; it never
...
**Capability declaration extension (summary — full schema in [template-composition-model.md](../foundations/template-composition-model.md)):**
```yaml
...
peer_dcm_capabilities:
  ...
  # tunnel wire contract (docs/spec/governance/accreditation-and-authorization-matrix.md) — "what the remote
...
the control plane at registration and receives a matched-capabilities answer (§10.2). This contract is
...
[Control-Plane Capabilities Matrix](https://github.com/croadfeldt/dcm/blob/main/architecture/the control plane-Capabilities-Matrix.md)
...
| `PRV-004` | Peer control-plane instances are treated as typed providers. Federation is the Provider abstraction applied across control-plane instances — not a separate abstraction. |
