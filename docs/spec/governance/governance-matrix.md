...
hand-authored field lock (`docs/spec/contracts/policy-contract.md` §10 — `field_locks` with
...
| `GMX-011` | A refusal to cross a sovereignty boundary is emitted as `policy.sovereignty_egress_denied` and names the **declared boundary** and the **refusing rule** (`governance_matrix_rule_uuid`), so the requester can seek an in-boundary alternative. The decision is taken on the structural surface — the target authority carried in the reference — without dereferencing the protected data (ADR-041's ruling that policy in its information-flow role matches the unresolved pointer, so sovereignty is enforced on the address, never by first reading what it points at). The refusal payload names no protected field and carries no protected value: a refusal is itself a boundary crossing and passes the same egress guard as any other payload ([`docs/spec/contracts/error-model.md`](../contracts/error-model.md) §8a). Timing is `GMX-009`'s — before dispatch, before transmission, before serialization — because a refusal issued after the crossing is a disclosure that cannot be withdrawn. **The datum exists: it is the URF authority axis.** A reference is a URF (`identifier-scheme.md` §9), and `//authority/` is one of its five axes — elided when the target is local, present when it is a peer's. So the authority this rule decides on is carried by the reference itself and is readable without dereferencing anything, which is exactly the structural surface the rule requires. The earlier telling of this row described `target_authority` as a missing field on a six-field `Reference` object; that object was replaced by the URF grammar, where each of its fields became an axis — the authority was not dropped, it moved. |
