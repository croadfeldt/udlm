...
In many cases they are the same team: a networking team may both define `Network.VLAN` as the Resource Type Authority AND register as the Service Provider that realizes VLANs. In other cases they are different: a platform team defines `Machine.VM` as the Resource Type Authority, and multiple compute providers each independently register Catalog Items implementing that specification.
