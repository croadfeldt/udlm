...
| `encryption_key` | `Credential.EncryptionKey` | Data-at-rest encryption — per-tenant DEK wrapped by a KEK (envelope encryption); the addressable key a Tenant's `key_bindings` reference and the **crypto-shredding** primitive (destroy the KEK → tenant data unrecoverable; `bundle.schema.json` `key_bindings`, GRP-013) | Per algorithm defaults | Pre-expiry, on rotation, or on tenant offboarding |
...
([`docs/spec/foundations/four-states.md`](../foundations/four-states.md) — Intent is the immutable record
...
refusal-payload rule ([`docs/spec/contracts/error-model.md`](../contracts/error-model.md) §8a — a refusal
...
UDLM defers algorithm specifics to the [Standards Catalog](../../../registry/standards-catalog.md). The substrate enforces the following invariants:
