...
- **Machine-validatable surface** — the model is only as solid as its schemas. Current: type
  ...
  `docs/spec/foundations/composite-service-model.md` §1.4/§2.5), (3) policy record + typed outputs **[DONE — registry/policy.schema.json; type-spec outputs are the referenceable typed binding surface, output-resolved by the validator]**, (4) layer record **[DONE — registry/layer.schema.json]**, (5) audit record +
