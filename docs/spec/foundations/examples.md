...
```yaml
# intent-store/a1b2c3d4-tenant/Machine/VM/f5e6d7c8-entity/intent.yaml
...
- Policy pre-validation: no Validation Policy violations detected (4 CPU is within AppTeam's quota)
...
```yaml
# Validation policy (compliance-class): vm-size-limits evaluates
...
# Validation policy (compliance-class): approved-os-images evaluates (AppTeam's tenant policy)
...
```yaml
# requested-store/a1b2c3d4-tenant/Machine/VM/f5e6d7c8-entity/requested.yaml
...
```yaml
# Drift response policy for Machine.VM at significant severity:
