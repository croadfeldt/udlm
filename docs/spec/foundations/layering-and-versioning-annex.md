...
| 11 | What is the minimum validation review period for a proposed policy before it can be activated? | Policy governance | ✅ Resolved — Validation Policy (compliance-class)=14d, Validation Policy (operational-class)=7d, Transformation=3d × profile multiplier (homelab=0×, dev=0.5×, standard=1×, prod=1.5×, fsi/sovereign=2×); the control plane enforces; emergency bypass requires dual-approval audit (OPS-004) |
