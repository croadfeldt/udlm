...
Location layers are one application of the **Reference Data Layer** pattern (see [layering-and-versioning.md](layering-and-versioning.md)). The same pattern governs OS images, VM sizes, network zones, storage classes, and any other field where valid values are a governed, versioned, authority-owned set rather than a static list.
