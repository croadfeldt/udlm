...
**A resource type models the elements that bear the dependency graph and the audit trail; the provider projects the concrete configuration, and the control plane is the means to configure it. UDLM is a *conduit* for config, not a *modeler* of it.**
...
### 2. Provider-specific config is *extra data* — declared by the provider, configured through the control plane, stored as state

Beyond the portable definition, a provider may accept **extra, provider-specific config**. UDLM does not model its schema field-by-field — the provider **declares it** (`provider-contract.md` §1a.3 config-projection: none → text passthrough; typed → a typed interface), and **control-plane projects a configuration interface** so the consumer sets it *through the control plane* — a real configuration channel across the config lifecycle, not an opaque dead-drop. The provider owns the *schema*.

The **values** are stored as provider-scoped state — a **Provider-Class `SharedDataElement`** (ADR-038; the retired `provider_extensions` carrier is validator-rejected) — across Requested and Realized, governed like any state (audit, provenance, tenancy) and **portability-flagged** (`portability_breaking: true`, consumer notified — relying on a vendor's extra config isn't portable to another provider; silent non-portability is prohibited). So the two tiers are **portable base** (every provider satisfies it) → **provider-specific extra** (this provider, stored as a Provider-Class `SharedDataElement`), and **the control plane stores both** (§3). The concrete *mechanism* never enters the substrate (naturalization, DCM ADR-023); the config *state* always does.

**Scope — where UDLM stops.** UDLM defines only **(a) the base resource type** and **(b) the extension model** (Provider-Class `SharedDataElement`s, ADR-038) that carries provider non-base data. The config **editor** is **pure the control plane**: where a provider has a native editor for a thing (OpenShift's, for an OCP-backed resource), the control plane **delegates to it**; where it does not, the control plane exposes a **generic text editor and enforces the provider-declared schema** on it. That is the *implementation* of config-projection (§1a.3), not the data model — named here only to place the boundary; it is specified control-plane-side. **A delegated editor is never an audit bypass:** the contract binds a provider that exposes its own editor to **report the resulting realized updates back** and to support the control plane's **before-and-after actor validation**, so audit / observability / provenance — **and the sovereignty hard-gate and tenant isolation** — hold at all times even when the edit happens in the provider's UI: the config tooling operates inside the control plane's governance, never around it (`provider-contract.md` §1a.3).

### 3. The control plane stores the config *state* — it is the state system-of-record

Ownership of the config *schema* is the provider's (§2). Ownership of the config *state* — the values across Requested / Realized / Discovered — is **the control plane's**, and the two must not be conflated. *"The provider owns config"* is true of the schema, **not** of the state.
...
- **Project + configure via control plane, don't model:** command/args, restart policy, replicas, security context, the runtime's remaining knobs — the provider declares them, control-plane projects the interface, the consumer sets them, they land in the since-retired `provider_extensions` state, portability-flagged.
...
- A richer provider gives a richer configuration interface **without a spec change** — config depth is the provider's to offer and the control plane's to project.
- Portability is honest: reliance on provider-exposed config is flagged, never silent (resource-type-hierarchy §1 Transparency — a provider-specific element marks the entity `portability_breaking` and the consumer is notified before/at realization).
