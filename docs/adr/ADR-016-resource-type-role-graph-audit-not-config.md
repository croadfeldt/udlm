...
The **values** are stored as provider-scoped state — a **Provider-Class `SharedDataElement`** (ADR-038; the retired `provider_extensions` carrier, `PRV-010`, is validator-rejected) — across Requested and Realized, governed like any state (audit, provenance, tenancy) and **portability-flagged** (`portability_breaking: true`, consumer notified — relying on a vendor's extra config isn't portable to another provider; silent non-portability is prohibited). So the two tiers are **portable base** (every provider satisfies it) → **provider-specific extra** (this provider, stored as a Provider-Class `SharedDataElement`), and **DCM stores both** (§3). The concrete *mechanism* never enters the substrate (naturalization, DCM ADR-023); the config *state* always does.
