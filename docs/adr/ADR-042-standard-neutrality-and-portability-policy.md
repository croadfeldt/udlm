# UDLM ADR-042: Standard neutrality as a derived property + a profile-dialed portability policy — enable, don't judge
...
**Settles:** how UDLM steers `adopts[]` toward vendor-neutral standards **without judging an authority's choice
...
derivable* property read off its **governing body**, not a verdict; (2) **policy may evaluate** that property at
...
But **UDLM cannot and must not police an authority's decision** to adopt one. Authorities author under their own
...
authoring choice as a defect. The principle: **we enable, we don't judge; document best practice, don't mandate.**
...
to exactly the judging we rejected. This ADR resolves that.

## Decision

Split the one conflated "approved list" into **fact**, **mechanism**, and **stance** — no layer is UDLM judging.
...
*about the standard* (adopt-by-reference), **not a curated allowlist and not a verdict on any adoption.**
- **Derive, don't store** (ADR-027 addendum discipline): `neutrality` is *computed* from the standard's
  governance, not a hand-set flag that drifts and re-imports the curation/verdict problem.
...
**UDLM classifies the standard (describes it, from its governance); the org judges its own estate (opt-in
profile).** An approved-standards *list* is UDLM deciding for everyone — judging. A *derivable property + a
profile knob* is each org deciding for itself — enabling. It reuses the ADR-041 firewall machinery (a policy
...
- **UDLM ships an approved-standards allowlist / prohibits provider-native `adopts[]` globally** — rejected: UDLM
  judging for everyone; unanswerable "whose list?"; polices legitimate authoring (VMs are commonly KubeVirt); the
  exact trap this ADR exists to avoid.
- **Store `neutrality` as an authored per-standard flag** — rejected in favor of deriving it from the governing
  body: a hand-set flag drifts and reintroduces the curation/verdict burden (derive-don't-store).
