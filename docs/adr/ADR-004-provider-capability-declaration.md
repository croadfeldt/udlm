...
### 4a. What the capability's sovereignty block carries (plane, conformance, verifiable attestation)

Three additions keep the per-capability sovereignty stance enforceable end-to-end; the mechanism and match rules live in `governance/accreditation-and-authorization-matrix.md` (this ADR only records that the declaration carries them):

- **`enforcement_plane` (`data` | `control` | `both`)** — sovereignty splits along the plane that leaks (matrix §3.8): *control-plane* (who can operate/see telemetry) vs *data-plane* (where the bytes rest). UDLM/DCM is a declaration-and-placement layer, not a byte-level enforcer — so for a data-plane requirement DCM **conveys the requirement + the execution-slice** to the enforcing provider and **verifies that provider's data-plane attestation**; the provider enforces residency of the bytes. The stance and the accreditation each declare their plane; a data-plane requirement is only satisfied by a data-plane (or `both`) attestation.
- **`conformance_claims[]` (`framework` + optional `level`/`statement`)** — the provider's **self-declaration** of standards it adheres to (ISO 27001, SOC 2, GDPR, …), declarable on the provider default and per category. A claim is **self-asserted until an accreditation attests that framework** — the same claim→attestation escalation as sovereignty, generalized (matrix §3.7). This is our conformance register (Gaia-X self-description / OSCAL SSP lineage).
- **Verifiable accreditation** — an accreditation is a **verifiable credential**: it carries a `proof` (the accreditor's signature) and a `trust_anchor` (CAB / eIDAS TSP / government registry / DID / X.509). DCM applies a **two-gate** decision — **(1) verify** the proof + chain + currency, then **(2) appraise** the 1-1 scope match (matrix §3.7). Verification failure and scope-mismatch are distinct outcomes; neither alone grants trust.
