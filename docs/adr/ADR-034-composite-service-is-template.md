...
- **The name.** `Template`, single name, no dual naming. "Composite Service" is retired rather than kept as an alias: one concept had grown three names (`Composite Service`, the `composite` group class, `Template`), and the third existed only because the first two were never removed.
- **Resources-only Template — no marker.** A constituent carries `edge_type` (`contained_by` | `binds_to`), so "resources-only" is exactly *zero `binds_to` constituents*, readable from the composition itself. The shape stays derived (ADR-027 addendum), and a marker would store what the constituent list already answers (DRV-001).
