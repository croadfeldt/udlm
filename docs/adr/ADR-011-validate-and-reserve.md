...
- **Data scoping is preserved across the split.** Each verb is an ordinary governed the control plane→Provider crossing carrying only the `role: execution` slice; the Governance Matrix fires at every crossing (`docs/spec/contracts/data-roles.md`; `PRV-008`; provider-contract §6a). The two-phase model adds request *types*, not new data-exposure paths.
...
- **−** the control plane must carry reconcile-loop state (round count, wall-clock, per-hold TTLs) and enforce a budget; granted TTLs and the budget must be mutually coherent, or a slow provider's hold expires before the barrier and manufactures a stalemate.
