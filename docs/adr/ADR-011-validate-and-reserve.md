...
But the concept was **narrowed and orphaned**. The `trim: move operational/runtime leaks out of UDLM (placement, caches, capacity)` change (commit `2c48a44`) correctly moved *placement orchestration* to DCM-runtime — and in doing so demoted reserve to a **provider-selection** step ("which provider can host"), dropping the parts that are genuinely UDLM **wire contract**: the **reserve/commit/release provider verbs**, the **reservation-hold object**, and the **whole-graph commit barrier**. Its home decision (placement) lives on the DCM side (DCM ADR-019); reserve was left with no UDLM **contract** or decision of record. So reserve survives as a timeout and a `placement.yaml` field with no contract and no decision of record. This ADR restores the contract half without re-importing the orchestration DCM correctly owns.
...
### 6. The reserve loop is bounded, multi-round negotiation — and stalemate is a first-class terminal outcome

Reconciliation is **not assumed to converge in one pass, or quickly.** Reserving a dependency returns facts that can shift another dependency's criteria, whose re-reservation can shift the first — an **iterative negotiation** between providers that takes real time (each reserve is a round-trip; a provider may counter with what it *can* hold, not what was asked). The model states this explicitly:

- **Multiple rounds are expected.** DCM re-reserves and recomputes across as many rounds as it takes for the holds to become mutually consistent. TTL **renewal** (§3) exists precisely for this — a good hold is kept alive while slower dependents catch up, so negotiation does not lose ground it already gained.
- **The loop is bounded.** A profile-governed **reconcile budget** — a maximum number of rounds *and* a maximum wall-clock, with backoff (`reservation_reconcile_budget`, `lifecycle/operational-models.md` §2) — caps the negotiation. This is the reserve-phase instance of ADR-006's bounded-convergence rule: the loop MUST converge or terminate, never oscillate indefinitely.
- **Stalemate is a distinct terminal outcome.** If the budget is exhausted with no fixed point — mutually unsatisfiable criteria, oscillation, or holds that cannot be kept valid long enough to converge — DCM terminates with **`RESERVATION_RECONCILE_STALEMATE`**, *distinct from* `RESERVE_QUERY_ALL_EXHAUSTED` (that is **no capacity**; this is **no agreement**). It **releases every hold** (nothing was built) and **surfaces the failure** via `RELEASE_ALL_AND_SURFACE` — re-plan, `NOTIFY_AND_WAIT` for human negotiation, or `ESCALATE`, per policy. A stalemate costs only dropped holds, never a teardown.
- **A slow provider can end the loop early — attributably (no new mechanism).** A provider's granted reserve TTL is itself a factor in convergence: if that provider is slow and its hold expires mid-negotiation and **cannot be renewed** (§3), the loop may end before it reaches the barrier. This needs nothing extra — the responsible provider is **already identified** by the `reservation.expired` / `reservation.expiry_unconfirmed` event's `provider_uuid`, so an early termination is **attributable, never anonymous**. (The corollary is a tuning obligation, below: granted TTLs must be sized to survive the negotiation via renewal.)
...
- **+** Reconciliation is **honestly modeled** as bounded multi-round negotiation with a **distinct stalemate terminal** (`RESERVATION_RECONCILE_STALEMATE` ≠ `RESERVE_QUERY_ALL_EXHAUSTED`) — not assumed to converge in one pass. A stalemate costs only dropped holds and surfaces for re-plan or human negotiation.
- **−** DCM must carry reconcile-loop state (round count, wall-clock, per-hold TTLs) and enforce a profile-governed budget; convergence is not free and the budget must be tuned so legitimate slow negotiations are not cut off prematurely. Granted TTLs (`min_hold_ttl` / `max_hold_ttl`) and the reconcile budget must be **mutually coherent** — a hold must be renewable long enough to span the negotiation, or a slow provider's hold expires before the barrier and manufactures a stalemate.
