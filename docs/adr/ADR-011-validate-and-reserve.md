...
- **−** DCM must carry reconcile-loop state (round count, wall-clock, per-hold TTLs) and enforce a profile-governed budget; convergence is not free and the budget must be tuned so legitimate slow negotiations are not cut off prematurely.
