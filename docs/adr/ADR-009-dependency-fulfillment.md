...
**Why this is required in advance.** The purpose is to move the provider-to-provider agreement to **design/registration time, never execution time**. Two providers that must exchange information across a boundary — the broker and the resource's owner — need to agree on the *shape* of that information **before anything is realized**. A base-type extension (a) or a custom type (b) makes that shape **declared and machine-validatable up front**, so DCM verifies at admission that the fields the broker will send are fields the target actually accepts. If the shape were only discovered at realization, a mismatch — a field the target cannot parse, a constraint it cannot meet — becomes a **run-time failure**: a half-realized VM holding an IP it cannot bind. Fixing the exchange contract ahead of execution means realization carries out a **pre-validated exchange whose outcome is known-true before it runs** — a pre-destined success, not a run-time gamble. This is 'valid by construction' applied across the provider boundary.
