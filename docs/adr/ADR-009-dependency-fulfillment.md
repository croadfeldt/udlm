...
*Release / expiry / stalemate.* Any hold not committed — validation failure, cancellation, **TTL expiry** (an implied release that emits `reservation.expired`; DCM's watchdog backstops a silent provider), or a **reconcile stalemate** (the bounded multi-round negotiation exhausted its budget without converging — `RESERVATION_RECONCILE_STALEMATE`) — is released. Because the reserve phase built nothing, **no half-built resource is ever left behind.**
