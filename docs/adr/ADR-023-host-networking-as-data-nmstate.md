...
**Background — read first (the cold reader's on-ramp; skip if you have the context).** `docs/guides/host-network-and-config-model.md` (the ratified mapping this records — the standards survey, the per-type model, and the worked example); ADR-013 (hardware component scope — the adapter is a kept component); `docs/spec/principles/adopted-standards.md` (Tier-2 adopt-by-reference); the affected types `network.ip-address`, `hardware.network-interface`, `network.dhcp-scope`, `network.address-service`, and the new `network.connection-profile`
...
> This ADR records the **decision + grounding**; the concrete per-type mapping and worked example live in `docs/guides/host-network-and-config-model.md` (single-source, SPEC-DESIGN §33). It records that proposal as the decision (previously "not yet an ADR — react before ratifying").
