...
- **ADR-008** (the UDLM/DCM boundary): access **determination** is external; UDLM carries the data and the contract, never the decision procedure.
- **The drift ruling** (2026-07): neither UDLM nor DCM has a built-in action for a drift finding — policies dictate what happens. Same shape: the model carries the facts, the decision lives elsewhere.
...
2. **The evaluation context** — what DCM sends the engine (§7.1).
...
**Out of scope, tracked separately.** A layer's `activation_condition` and a conditional dependency's `condition` carry predicates too, evaluated by DCM's *assembly* engine rather than a policy engine. Same boundary question, separate ruling — neither is schema-backed today (the only schema mention is the exclusion-reason enum value `activation_condition_false`), so nothing is load-bearing while it waits.
