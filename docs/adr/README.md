...
| [042](ADR-042-standard-neutrality-and-portability-policy.md) | Standard neutrality + portability policy (enable, don't mandate) — a standard's `neutrality` is *derived* from its governing body (multi-vendor → vendor-neutral; single-project → provider-native), *not* an approved-standards list; policy *may* evaluate it at the adoption crossing (ADR-041 firewall); an org **profile dials** the stance `off`(default)/`warn`/`deny`, scoped to its own estate. UDLM classifies the standard; the org governs its own estate. No new rule family | Proposed |
