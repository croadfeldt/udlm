...
A provider **acting, changing, or denying** a request — and drift, and any other Data state change — is an **Event** that re-triggers policy evaluation. Policy is never a one-shot gate: it is re-evaluated on every provider outcome and every state change. Provider denial is not a dead-end; it re-enters the loop (re-plan, alternative placement, or surface a failure). Re-entrancy extends into the **reserve-phase reconciliation loop** (ADR-011): as holds land and yield new reserved facts, policies re-evaluate against the enriching reserved graph — and a policy **declares whether it participates** in that loop or is evaluated once at the commit barrier (`registry/policy.schema.json` `reconciliation`; policy-contract §7.6).
