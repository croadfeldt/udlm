...
- Four families (ADR-027): **Resource** and **Process** — maintained state vs bounded
  execution — each carrying `entity_type: single | multi` (the coarse shape from DCM's
...
| entity_type (shape) | single, multi — Resource & Process only | entity-types §2.2 (ADR-027) |
