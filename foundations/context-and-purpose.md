...
| 3 | Which cache is authoritative when caches diverge? | Conflict resolution | ✅ Resolved — the authoritative state store (the single PostgreSQL-compatible database, D1) always wins; caches are projections; divergence triggers rebuild from the authoritative store (CACHE-003) |
...
| 6 | How are the four states represented physically? | Physical model | ✅ Resolved — all four state domains live in a single PostgreSQL-compatible database ([data-model-core](data-model-core.md) §6, ruling D1; [four-states](four-states.md) §4); Git and event streams are ingress/carrier profiles, not the system of record. *(Supersedes the earlier STO-005 resolution — Intent/Requested in Git, Realized in an Event Stream.)* |
...
- **GitOps** — an optional ingress/carrier profile: consumers who prefer PR-based workflows can submit intent via Git, gaining review and change-tracking ergonomics. Git is an ingress path, not a state store — the canonical physical substrate for all four state domains is a single PostgreSQL-compatible database ([data-model-core](data-model-core.md) §6, ruling D1)
