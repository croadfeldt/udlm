...
| **Access** | Identity — who/what a request acts as | DCM | `Identity` (+ future identity-contract types) — [Auth Providers & Identity Contract](../governance/auth-providers.md); `Escrow` — identity state surviving host re-realization, [Identity Escrow](../entities/identity-escrow.md) | Contract: identity is resolved/verified by DCM, never held as a maintained state |
