...
The registry holds **53 types** (Access 7, Knowledge 5, Process 2, Resource 39). Every spec is strict (`additionalProperties: false`, 53/53 (100%)) and the instance-fuzz harness rejected 4446 of 4446 adversarial mutations (100.00% discrimination density, 0 open finding(s)). 35/53 (66%) of types appear in at least one use case; 18 appear in none. 5 types are named by a specific consumer manifest; the other 48 are carried only by the 4 envelope-level (all-types) consumers. 8 types declare no outputs and 24 declare exactly one — the thinnest part of the binding surface. Three metrics are owned by other systems and report null until those systems land (table at the end).

## Headline

| Metric | Value | Reading |
|---|---|---|
| Types (by family) | 53 (Access 7, Knowledge 5, Process 2, Resource 39) | — |
| Discrimination density | 4446/4446 = 100.00% | mutations rejected / attempted; 0 finding(s) |
| Strictness coverage | 53/53 (100%) | asserted — a non-strict spec fails this tool |
| Outputs adequacy | 8 zero-output, 24 one-output | declared Realized binding surface |
| Context coverage | 53/53 (100%) | plain-English `context` blocks |
| Relationships coverage | 44/53 (83%) | types declaring `relationships[]` |
| UC coverage | 35/53 (66%) | types appearing in >=1 use case (149 UC files scanned) |
| Consumer coverage | 53/53 (100%) | ADR-044 manifests; 5 named explicitly, rest via all-types consumers |
...
**Zero-output types (8):** `Access.Grouping`, `Grouping.Authorization`, `Grouping.Tenant`, `Hardware.GraphicsProcessor`, `Hardware.Processor`, `SoftwareImage`, `SoftwarePackage`, `Vulnerability`
...
Types appearing in no use case (18) — each is either ahead of its
...
- `relationships[]` missing (9): `Access.Grouping`, `Automation.OSPatch`, `Facility.PowerFeed`, `Grouping.Authorization`, `Grouping.Tenant`, `Observability.LogShipper`, `Storage.Layout`, `Topology`, `Vulnerability`
