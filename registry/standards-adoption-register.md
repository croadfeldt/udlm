...
**Default decider attribution:** Chris Roadfeldt (recorded session rulings), research + drafting
...
**Why:** the vendor-neutral hardware-as-asset vocabulary, and the one our estate's producers actually speak (BMCs: iDRAC, Supermicro — the cartman provisioning path). *Alternatives:* IPMI (no data model), DMTF CIM (superseded by Redfish for REST-era use — PRIOR-ART). **License:** DMTF — compatible-reference.
...
**Why:** RFC 2131 defines the protocol concepts; Kea is the estate's actual DHCP implementation (post-pfSense migration) and its config vocabulary names the operational objects. Kea is MPL-2.0 → **reference-only** verdict (vocabulary referenced, no text reproduction); RFC — compatible-reference.
...
**Why:** the estate's actual UPS telemetry producer (upsd/upsmon on jinx + kyle); its status vocabulary is the de-facto open standard. **License:** GPL — reference-only.
