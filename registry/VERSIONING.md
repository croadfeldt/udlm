...
|---|---|---|---|---|
| 2026-07-25 | 41 types REVISION-bumped (every resource type except `Facility.Location`, `Hardware.BiosProfile`, `Network.Gateway`, `Platform.Hub`, `SoftwarePackage`, `Storage.FileShare`) | **Context-repair wave** (interpretability-probe campaign): `context` prose aligned to the schemas it describes — required properties named (`actor_type`, `pool_kind`, `service_kind`, `host_type`, `scope`, `name`, `handle`, …), schema spellings carried in prose (`curation_state`, `allocation`, `zone_type`, `segment_id`, `forward_mode`, `data_protection`, …), enum/casing/pattern traps disarmed (`vlan` for 802.1Q, `read_write_once`, `available`, CIDR-with-prefix, whole-number capacities), the `identity`-nesting convention stated on all six asset/component types, and three stale passages rewritten (Hardware.StorageDevice role placement, Network.IPAddress allocation vocabulary, Network.Switch management addressing). Context/prose-only — no schema, field, enum, or relationship change. | **No** — description-only; wire/instance format unaffected. | None required. |
